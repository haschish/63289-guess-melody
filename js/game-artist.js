...
const element = createElement(`section`, {className: `game game--artist`, innerHTML});
...
const radioInputs = element.querySelectorAll(`[type="radio"]`);
const onRadioInputsChange = () => {
  const randomInt = Math.ceil(Math.random() * 3);
  switch (randomInt) {
    case 2: changeScreen(failTime);
      break;
    case 3: changeScreen(failTries);
      break;
    default: changeScreen(resultSuccess);
  }
};
radioInputs.forEach((item) => item.addEventListener(`change`, onRadioInputsChange));
